  ...
  const ensureIndex = (vaultRoot: string) => {
    ...
    for (const note of all) {
      ...
      if (note.frontmatter?.type === "article") {
        ...
        if (key) articleCounts.set(key, (articleCounts.get(key) ?? 0) + 1)
      }
    }
  }
  ...
  return {
    ...
    markdownPlugins(ctx) {
      return [() => (tree: any, file: any) => {
        ...
        const sourceSlug = simplifySlug(slugifyFilePath(relativeSource as any))
        const sourceSlugDirectory = path.posix.dirname(String(sourceSlug).replaceAll("\\", "/"))
        let authorHref = path.posix.relative(sourceSlugDirectory, String(author.slug).replaceAll("\\", "/"))
